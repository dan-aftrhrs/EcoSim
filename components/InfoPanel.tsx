...
import { Info, Radio, ChevronUp, ChevronDown } from 'lucide-react';
...
export const InfoPanel: React.FC<InfoPanelProps> = ({ currentConfig, onSpeciesAction, newsFeed }) => {
    ...
    return (
        <div className="flex flex-col lg:flex-col gap-2 lg:gap-4 h-64 lg:h-full w-full print:hidden relative overflow-hidden rounded-xl border border-slate-700 bg-slate-900/50">
            
            {/* NEWS FEED SECTION - Fills available space */}
            <div className={`flex flex-col w-full transition-all duration-300 ${isMobileExpanded ? 'opacity-0 pointer-events-none' : 'opacity-100 flex-1'}`}>
                ...
                <div className="overflow-y-auto custom-scrollbar p-2 flex-1 bg-slate-900/30 mb-14 lg:mb-0">
                    {logs.length > 0 ? (
            ...
            {/* SPECIES INTEL SECTION - Bottom Sheet on Mobile */}
            <div className={`
                bg-slate-800 border-t border-slate-700 shadow-[0_-5px_15px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden
                lg:static lg:h-full lg:w-full lg:flex-1 lg:shadow-none lg:border-t-0 lg:bg-transparent
                absolute bottom-0 left-0 w-full transition-all duration-300 ease-in-out
                ${isMobileExpanded ? 'h-full z-20' : 'h-14 z-10'}
            `}>
                 <div 
                    className="p-2 lg:p-3 border-b border-slate-700 bg-slate-800 flex-shrink-0 cursor-pointer lg:cursor-default flex justify-between items-center"
                 ...
                 <div className="flex flex-col h-full lg:hidden overflow-hidden bg-slate-900">
                    {/* Icon Selection Strip */}
                    <div className="flex overflow-x-auto p-2 bg-slate-900 border-b border-slate-700 gap-2 no-scrollbar shrink-0 justify-center">
                        {SPECIES_LIST.map(type => (
                            <button
                                ...
                                className={`p-2 rounded-lg flex items-center justify-center transition-colors ${selectedMobileSpecies === type ? 'bg-slate-700 ring-1 ring-blue-500' : 'bg-slate-800 hover:bg-slate-700'}`}
                            >
                    ...
                    <div className="p-4 overflow-y-auto custom-scrollbar flex-1 bg-slate-900/50">
                         <div className="flex items-center gap-2 mb-3 pb-2 border-b border-slate-800">
                             <div className="p-1.5 rounded bg-slate-800">
                                 <SpeciesIcon type={selectedMobileSpecies} />
                             </div>
                             <span className="text-sm font-bold" style={{ color: SPECIES_CONFIG[selectedMobileSpecies].color }}>
